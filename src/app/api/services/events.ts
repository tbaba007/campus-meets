...
export const GetAcceptedAvailableEvents=(id:number)=>{
    return GET({
        path:`marketplace/GetAcceptedAvailableEventsById/${id}`
    })
}
...
//UnJoinAcceptedEvent/
export const UnJoinAcceptedEvent= (id:number,Acceptedids:string ) =>{
    return PATCH({
        path:`marketplace/UnJoinAcceptedEvent/${id}`,
        payload:{Acceptedids}
    })
}
