...
const CreateEvent = () => {
  ...
  const [isEventCreated, setIsEventCreated] = useState(false);
   ...
   //new code for comparing time and date
   const today = date.toISOString().split("T")[0];
   const currentdate = new Date();
   const currentTime = currentdate.getTime();
   const startDateTime = new Date(`${state.StartDate}T${state.StartTime}:00`).getTime();
  ...
  const onCreate = async () => {
    ...
    const currentTime = currentdate.getTime();
    const startDateTime = new Date(`${state.StartDate}T${state.StartTime}:00`).getTime();
    const endDateTime = new Date(`${state.EndDate}T${state.EndTime}:00`).getTime();
    ...
    if(EndTime && StartTime){
      if(StartDate===today){
        ...
        if(currentTime>startDateTime){
          ...
          return toast.error("End Time cannot be lesser than Start Time")
        }
      }
    }
